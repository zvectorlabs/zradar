...
async fn main() -> Result<()> {
    ...
    let storage_path =
        std::env::var("STORAGE_PATH").unwrap_or_else(|_| "./data/trace-batches".to_string());
    ...
    let postgres_client = Arc::new(PostgresClient::from_config(&postgres_config).await?);

    // Create telemetry writer (uses TelemetryWriter trait for flexibility)
    let writer: Arc<dyn TelemetryWriter> =
        Arc::new(PostgresTelemetryRepository::new(postgres_client));
    ...
    let worker_pool = WorkerPool::new(num_workers, job_queue, block_storage, writer);
