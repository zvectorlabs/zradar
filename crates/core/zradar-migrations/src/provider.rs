...
pub trait MigrationProvider: Send + Sync {
    ...
    fn discover_migrations(&self) -> std::pin::Pin<Box<dyn std::future::Future<Output = anyhow::Result<Vec<MigrationInfo>>> + Send + '_>>;
    
    /// Apply a specific migration
    fn apply_migration<'a>(&'a self, migration: &'a MigrationInfo) -> std::pin::Pin<Box<dyn std::future::Future<Output = anyhow::Result<MigrationResult>> + Send + 'a>>;
    
    /// Verify migration checksum
    fn verify_migration<'a>(&'a self, migration: &'a MigrationInfo) -> std::pin::Pin<Box<dyn std::future::Future<Output = anyhow::Result<bool>> + Send + 'a>>;
}
