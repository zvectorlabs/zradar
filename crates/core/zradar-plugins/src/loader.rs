...
impl PluginLoader {
    ...
    pub async fn run_migrations(
        ...
        for plugin_name in &config.enabled {
            ...
            if let Err(e) = registry
                .run_plugin_migrations(plugin_name, &migration_opts)
                .await
            {
    ...
    #[cfg(feature = "dynamic-loading")]
    async fn load_dynamic_plugin(&self, name: &str, registry: &PluginRegistry) -> Result<()> {
        ...
        unsafe {
            let lib = Library::new(&lib_path).map_err(|e| {
                PluginError::LoadFailed(format!("Failed to load {}: {}", lib_path, e))
            })?;

            // Look for the register function
            let register_fn: Symbol<unsafe extern "C" fn(&PluginRegistry) -> bool> =
                lib.get(b"register_plugin").map_err(|e| {
