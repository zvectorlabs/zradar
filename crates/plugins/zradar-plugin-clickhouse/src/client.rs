...
impl ClickHouseClient {
    ...
    pub async fn from_config(config: &serde_json::Value) -> Result<Self> {
        ...
        // Test connection
        let result: Result<u8, clickhouse::error::Error> =
    ...
    pub async fn run_migrations<P: AsRef<std::path::Path>>(
        &self,
        _migrations_dir: P,
    ) -> anyhow::Result<()> {
    ...
    pub async fn verify_migrations<P: AsRef<std::path::Path>>(
        &self,
        _migrations_dir: P,
    ) -> anyhow::Result<bool> {
        // TODO: Implement using MigrationRegistry
    ...
    pub async fn insert_scores(&self, scores: &[EvaluationScore]) -> Result<()> {
        ...
        if self.test_mode {
            self.client
                .query("OPTIMIZE TABLE evaluation_scores FINAL SETTINGS mutations_sync=2")
                .execute()
                .await?;
    ...
    pub async fn get_trace_score_summary(
        ...
        let results: Vec<ScoreSummary> = self.client.query(&query).fetch_all().await?;
    ...
    pub async fn get_score_by_id(
        ...
        let scores: Vec<EvaluationScore> = self.client.query(&query).fetch_all().await?;

        Ok(scores.into_iter().next())
    }

    /// Soft delete a score
    pub async fn soft_delete_score(&self, project_id: Uuid, score_id: Uuid) -> Result<bool> {
        let query = format!(
