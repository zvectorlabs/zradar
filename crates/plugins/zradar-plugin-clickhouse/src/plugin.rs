...
use zradar_models::{Span, Metric, EvaluationScore};
use zradar_plugins::{
    Plugin, PluginMetadata, PluginType, ConfigField,
    TelemetryWriterPlugin, TelemetryReaderPlugin,
...
impl MigratablePlugin for ClickHousePlugin {
    async fn run_migrations(&self, options: &MigrationOptions) -> anyhow::Result<Vec<AppliedMigration>> {
        ...
        let result = client.run_migrations(&options.migrations_dir).await?;
    ...
    async fn verify_checksums(&self, options: &MigrationOptions) -> anyhow::Result<bool> {
        ...
        client.verify_migrations(&options.migrations_dir).await
...
impl ClickHousePlugin {
    ...
    async fn get_pending_migrations(&self, options: &MigrationOptions) -> anyhow::Result<Vec<String>> {
