...
impl ClickHouseTelemetryReader {
    ...
    pub async fn query_traces(&self, filters: serde_json::Value) -> anyhow::Result<serde_json::Value> {
        let project_id = filters["project_id"].as_str()
        ...
        let rows: Vec<TraceRow> = self.client.client()
            .query(&query)
            .fetch_all()
            .await?;
        
        let traces: Vec<serde_json::Value> = rows.iter().map(|r| {
            serde_json::json!({
                "trace_id": r.trace_id,
                "name": r.trace_name,
    ...
    pub async fn get_trace_detail(
        ...
        let rows: Vec<SpanRow> = self.client.client()
            .query(&query)
            .fetch_all()
            .await?;
    ...
    /// Query spans
    pub async fn query_spans(&self, filters: serde_json::Value) -> anyhow::Result<serde_json::Value> {
        let project_id = filters["project_id"].as_str()
            .ok_or_else(|| anyhow::anyhow!("project_id required"))?;
    ...
    /// Get analytics
    pub async fn get_analytics(&self, query: serde_json::Value) -> anyhow::Result<serde_json::Value> {
        let project_id = query["project_id"].as_str()
            .ok_or_else(|| anyhow::anyhow!("project_id required"))?;
        ...
        let rows: Vec<AnalyticsRow> = self.client.client()
            .query(&sql)
            .fetch_all()
            .await?;
