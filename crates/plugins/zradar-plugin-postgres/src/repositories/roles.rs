...
use zradar_traits::{
    RoleRepository, PermissionDefinition, CustomRole,
...
impl RoleRepository for PostgresRoleRepository {
    async fn get_permission_definitions(&self, scope: Option<&str>) -> anyhow::Result<Vec<PermissionDefinition>> {
        let query = if let Some(s) = scope {
            sqlx::query("SELECT * FROM permission_definitions WHERE scope = $1 ORDER BY id")
                .bind(s)
        } else {
        ...
        Ok(rows.iter().map(|r| PermissionDefinition {
            id: r.get("id"),
    ...
    async fn get_permission_definition(&self, id: &str) -> anyhow::Result<Option<PermissionDefinition>> {
        let row = sqlx::query(
            "SELECT * FROM permission_definitions WHERE id = $1"
        )
        .bind(id)
