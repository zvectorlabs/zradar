...
impl HybridQueue {
    ...
    pub fn new(
        ...
        tracing::info!(redis_url = redis_url, "Hybrid queue initialized");
...
impl JobQueue for HybridQueue {
    ...
    async fn dequeue(&self, worker_id: &str) -> anyhow::Result<Option<Job>> {
        ...
        // BLPOP with timeout
        let result: Option<(String, String)> = redis_conn.blpop("job_queue:pending", 5.0).await?;
    ...
    async fn fail(&self, job_id: Uuid, error: &str) -> anyhow::Result<()> {
        ...
        if record.retry_count < MAX_RETRIES {
            ...
            redis_conn
                .zadd::<_, _, _, ()>("job_queue:retry", job_message.to_string(), retry_at as f64)
                .await?;
