...
impl ProjectRateLimiter {
    ...
    pub fn check_and_record(&self, project_id: Uuid, limit_per_second: u64, records: u64) -> bool {
        ...
        let mut bucket = self.buckets.entry(project_id).or_insert(TokenBucket {
            tokens: capacity,
            last_refill: now,
        });
...
mod tests {
    ...
    fn allows_burst_up_to_twice_rate() {
        ...
        assert!(limiter.check_and_record(Uuid::new_v4(), 10, 20));
    }
    ...
    fn rejects_when_bucket_empty() {
        ...
        assert!(limiter.check_and_record(project_id, 10, 20));
        assert!(!limiter.check_and_record(project_id, 10, 1));
    }

    #[test]
    fn zero_rate_rejects() {
        let limiter = ProjectRateLimiter::new();
        assert!(!limiter.check_and_record(Uuid::new_v4(), 0, 1));
    }
}
