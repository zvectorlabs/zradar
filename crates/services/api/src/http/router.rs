...
use crate::api_keys::{handlers as apikey_handlers, service::ApiKeyService};
...
pub fn create_admin_router(
    ...
    Router::new()
        ...
        .merge(crate::users::router::router(
            auth_service,
            jwt_auth.clone(),
            user_storage.clone(),
        ))
        .merge(crate::organizations::router::router(
            org_service,
            jwt_auth.clone(),
            user_storage.clone(),
        ))
        .merge(crate::projects::router::router(
            project_service,
            jwt_auth.clone(),
            user_storage.clone(),
        ))
        .merge(crate::api_keys::router::router(
            api_key_service,
            jwt_auth.clone(),
            user_storage.clone(),
        ))
        .merge(crate::roles::router::router(
            role_service,
            jwt_auth.clone(),
            user_storage.clone(),
        ))
        .merge(crate::telemetry::router::router(
            query_service,
            jwt_auth.clone(),
            user_storage.clone(),
        ))
        .merge(crate::scores::router::router(
            scores_service,
            jwt_auth,
            user_storage,
        ))
