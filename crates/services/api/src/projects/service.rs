...
impl ProjectService {
    ...
    pub async fn create_project(
        ...
        if self.project_storage.get_project_by_slug(org_id, &req.slug).await?.is_some() {
    ...
    pub async fn add_member(
        ...
        let target_user = self.user_storage.get_user_by_email(&req.user_email).await?
        ...
        if let Some(_existing) = self.project_storage.get_member(project_id, target_user.id).await? {
            return Err(ControlError::Conflict("User is already a project member".to_string()));
        }
        
        // 4. Add member
        let member = self.project_storage.add_member(project_id, target_user.id, req.clone()).await?;
    ...
    pub async fn remove_member(
        ...
        self.project_storage.remove_member(project_id, target_user_id).await?;
