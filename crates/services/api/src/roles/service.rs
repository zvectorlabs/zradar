...
use super::types::{
    CustomRole, PermissionDefinition,
...
impl RoleService {
    ...
    pub async fn create_role(
        ...
        // Check permission
        self.rbac.check_permission(user_id, org_id, None, "admin:roles").await?;
        ...
        // Create role
        let role = self.role_storage.create_custom_role(org_id, req.clone(), user_id).await?;
    ...
    pub async fn list_roles(
        ...
        user_id: Uuid,
        org_id: Uuid,
    ) -> Result<Vec<CustomRole>> {
        // Check permission
        self.rbac.check_permission(user_id, org_id, None, "read:roles").await?;
    ...
    pub async fn get_role(
        ...
        let role = self.role_storage.get_custom_role(role_id).await?
            .ok_or(ControlError::NotFound("Role not found".to_string()))?;

        // Check permission
        self.rbac.check_permission(user_id, role.organization_id, None, "read:roles").await?;
    ...
    pub async fn update_role(
        ...
        let existing_role = self.role_storage.get_custom_role(role_id).await?
            .ok_or(ControlError::NotFound("Role not found".to_string()))?;

        // Check permission
        self.rbac.check_permission(user_id, existing_role.organization_id, None, "admin:roles").await?;
        ...
        // Update role
        let updated_role = self.role_storage.update_custom_role(role_id, req.clone()).await?;
    ...
    pub async fn delete_role(
        ...
        // Check permission
        self.rbac.check_permission(user_id, role.organization_id, None, "admin:roles").await?;
