...
impl ApiClient {
    ...
    pub async fn get_with_header(&self, path: &str, header_name: &str, header_value: &str) -> Result<Response> {
        let mut req = self.client.get(self.build_url(path))
    ...
    pub async fn get_organization(&self, org_id: &Uuid) -> Result<Value> {
        let response = self.get(&format!("/api/v1/organizations/{}", org_id)).await?;
        Self::get_json(response).await
    }
    ...
    pub async fn list_organizations(&self) -> Result<Vec<Value>> {
        ...
        response.json().await.context("Failed to parse organizations list")
    }
    ...
    pub async fn delete_organization(&self, org_id: &Uuid) -> Result<()> {
        let response = self.delete(&format!("/api/v1/organizations/{}", org_id)).await?;
    ...
    pub async fn create_project(&self, org_id: &Uuid, name: &str, display_name: &str) -> Result<Value> {
    ...
    pub async fn get_project(&self, project_id: &Uuid) -> Result<Value> {
        let response = self.get(&format!("/api/v1/projects/{}", project_id)).await?;
        Self::get_json(response).await
    }
    ...
    pub async fn list_projects(&self, org_id: Option<&Uuid>) -> Result<Vec<Value>> {
        ...
        response.json().await.context("Failed to parse projects list")
    }
    ...
    pub async fn delete_project(&self, project_id: &Uuid) -> Result<()> {
        let response = self.delete(&format!("/api/v1/projects/{}", project_id)).await?;
        Self::assert_status(&response, StatusCode::NO_CONTENT)
    }
    ...
    /// Create API key
    pub async fn create_api_key(&self, project_id: &Uuid, name: &str, description: &str) -> Result<Value> {
        let response = self.post(
    ...
    pub async fn list_api_keys(&self, project_id: Option<&Uuid>) -> Result<Vec<Value>> {
        ...
        let response = self.get(&path).await?;
        response.json().await.context("Failed to parse API keys list")
    }
