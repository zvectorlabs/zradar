//! Test scenarios organized by feature area
#![allow(unused_mut)] // dual_transport_test! macro may not use all env parameters in all variants

pub mod test_analytics;
pub mod test_audit_logging;
pub mod test_compression;
pub mod test_e2e;
pub mod test_health;
pub mod test_logs;
pub mod test_metrics;
pub mod test_parquet_metadata;
pub mod test_policy_enforcement;
pub mod test_query_api;
pub mod test_retention;
pub mod test_span_types;
pub mod test_telemetry_storage;
pub mod test_agent_analytics;
pub mod test_agent_load;
pub mod test_nemo_phase1;
pub mod test_nemo_phase2;
pub mod test_nemo_phase4;
pub mod test_e2e_fixtures;
pub mod test_nemo_gap_fixes;
pub mod test_nim_collector;
pub mod test_security;
pub mod test_tracing;
