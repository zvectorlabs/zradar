...
async fn test_get_api_key_by_id() -> Result<()> {
    ...
    let project = client.create_project(&org_id, &fixture.project_name, &fixture.project_display_name).await?;
    let project_id = parse_uuid_from_json(&project, "id")?;
    let created = client.create_api_key(&project_id, &fixture.api_key_name, &fixture.api_key_description).await?;
...
async fn test_list_api_keys() -> Result<()> {
    ...
    let _key1 = client.create_api_key(&project_id, &format!("{}-1", fixture.api_key_name), "Key 1").await?;
    let _key2 = client.create_api_key(&project_id, &format!("{}-2", fixture.api_key_name), "Key 2").await?;
    
    // List keys for project
    let project_keys = client.list_api_keys(Some(&project_id)).await?;
    assert!(project_keys.len() >= 2, "Should have at least 2 keys for project");
