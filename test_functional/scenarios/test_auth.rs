...
async fn test_login_with_valid_credentials() -> Result<()> {
    ...
    let token = client.login(&ctx.config.admin_email, &ctx.config.admin_password).await?;
...
async fn test_login_with_invalid_credentials() -> Result<()> {
    ...
    assert!(result.is_err(), "Login should fail with invalid credentials");
...
async fn test_token_required_for_protected_resources() -> Result<()> {
    ...
    let result = unauth_client.create_organization(&fixture.org_name, &fixture.org_display_name).await;
