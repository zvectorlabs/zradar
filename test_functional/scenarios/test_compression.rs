//! Compression tests - Verify gzip works for both gRPC and HTTP
...
async fn test_http_gzip_compression() -> Result<()> {
    ...
    let response = client.get_with_header(&path, "Accept-Encoding", "gzip").await?;
...
async fn test_compression_with_real_trace() -> Result<()> {
    ...
    let response = client.get_with_header(&query_path, "Accept-Encoding", "gzip").await?;
