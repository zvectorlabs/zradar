...
async fn test_multi_service_observability() -> Result<()> {
    ...
    let services = vec![
        ("frontend", vec!["render.page", "fetch.data", "click.button"]),
        ("backend-api", vec!["handle.request", "query.database", "format.response"]),
...
async fn test_multi_tenant_isolation() -> Result<()> {
    ...
    let org1 = client.create_organization(&tenant1_name, "Tenant 1").await?;
    let org1_id = parse_uuid_from_json(&org1, "id")?;
    let proj1 = client.create_project(&org1_id, &format!("{}-proj", tenant1_name), "Tenant 1 Project").await?;
    let _proj1_id = parse_uuid_from_json(&proj1, "id")?;
    
    println!("Creating Tenant 2...");
    let org2 = client.create_organization(&tenant2_name, "Tenant 2").await?;
    let org2_id = parse_uuid_from_json(&org2, "id")?;
    let proj2 = client.create_project(&org2_id, &format!("{}-proj", tenant2_name), "Tenant 2 Project").await?;
...
async fn test_api_key_lifecycle_management() -> Result<()> {
    ...
    assert_eq!(revoked["is_active"], false, "Revoked key should be inactive");
    ...
    assert_eq!(active_key["is_active"], true, "Active key should remain active");
...
async fn test_error_handling_and_recovery() -> Result<()> {
    ...
    println!("\nTest 2: Duplicate names");
    let org = client.create_organization(&fixture.org_name, &fixture.org_display_name).await?;
    let result2 = client.create_organization(&fixture.org_name, "Different Display Name").await;
    ...
    let result3 = client.create_project(&fake_org_id, "test-proj", "Test Project").await;
    ...
    let org_id = parse_uuid_from_json(&org, "id")?;
    let _project = client.create_project(&org_id, &fixture.project_name, &fixture.project_display_name).await?;
