...
async fn test_get_organization_by_id() -> Result<()> {
    ...
    // Create organization
    let created = client.create_organization(&fixture.org_name, &fixture.org_display_name).await?;
...
async fn test_list_organizations() -> Result<()> {
    ...
    for (i, org) in orgs.iter().enumerate() {
        println!("🔍 Org {}: Full JSON = {}", i, serde_json::to_string_pretty(org).unwrap_or_else(|_| "error".to_string()));
        if i >= 2 { break; } // Only print first 3
    }
    
    assert_not_empty(&orgs, "Should have at least one organization")?;
    
    // Find our created organization
    let found = orgs.iter().any(|org| {
        org["organization"]["id"].as_str().unwrap_or("") == created_id.to_string()
    });
...
async fn test_update_organization() -> Result<()> {
    ...
    let updated = client.update_organization(&org_id, &new_display_name).await?;
...
async fn test_duplicate_organization_name_rejected() -> Result<()> {
    ...
    let first_result = client.create_organization(&fixture.org_name, &fixture.org_display_name).await;
    assert!(first_result.is_ok(), "First org should be created");
    
    // Try to create another with same name
    let second_result = client.create_organization(&fixture.org_name, "Different Display Name").await;
    assert!(second_result.is_err(), "Duplicate org name should be rejected");
...
async fn test_organization_requires_authentication() -> Result<()> {
    ...
    let result = unauth_client.create_organization("test-org", "Test Org").await;
