...
async fn test_get_project_by_id() -> Result<()> {
    ...
    let org = client.create_organization(&fixture.org_name, &fixture.org_display_name).await?;
    let org_id = parse_uuid_from_json(&org, "id")?;
    let created = client.create_project(&org_id, &fixture.project_name, &fixture.project_display_name).await?;
...
async fn test_list_projects() -> Result<()> {
    ...
    let found = projects.iter().any(|p| {
        p["id"].as_str().unwrap_or("") == created_id.to_string()
    });
...
async fn test_list_projects_by_organization() -> Result<()> {
    ...
    let proj1 = client.create_project(&org_id, &format!("{}-1", fixture.project_name), "Project 1").await?;
    let proj2 = client.create_project(&org_id, &format!("{}-2", fixture.project_name), "Project 2").await?;
...
async fn test_update_project() -> Result<()> {
    ...
    let updated = client.update_project(&project_id, &new_display_name).await?;
...
async fn test_add_project_member_by_email() -> Result<()> {
    ...
    // Setup: Create org and project
    let org = admin_client.create_organization(&fixture.org_name, &fixture.org_display_name).await?;
    let org_id = parse_uuid_from_json(&org, "id")?;
    let project = admin_client.create_project(&org_id, &fixture.project_name, &fixture.project_display_name).await?;
    let project_id = parse_uuid_from_json(&project, "id")?;
    
    // Create a second user
    let member_email = format!("member-{}@test.com", uuid::Uuid::new_v4());
    let member_password = "SecurePassword123!";
    let register_response = admin_client.register(&member_email, member_password, "Test Member").await?;
    ...
    let add_response = admin_client.post(
    ...
    assert_eq!(status, 201, "Expected 201 Created, got {}: {}", status, response_text);
    ...
    let list_response = admin_client.get(
        &format!("/api/v1/projects/{}/members", project_id)
    ).await?;
    ...
    println!("Project members: {}", serde_json::to_string_pretty(&members_list)?);
    
    // Should have at least one member (the one we just added)
    assert!(members.len() >= 1, "Expected at least 1 member, got {}", members.len());
    
    // Find our added member
    let found_member = members.iter().find(|m| {
    ...
    let duplicate_response = admin_client.post(
        &format!("/api/v1/projects/{}/members", project_id),
        &add_member_body
    ).await?;
    ...
    assert_eq!(duplicate_status, 409, "Expected 409 Conflict for duplicate member");
    ...
    let invalid_response = admin_client.post(
        &format!("/api/v1/projects/{}/members", project_id),
        &invalid_email_body
    ).await?;
    ...
    assert_eq!(invalid_status, 404, "Expected 404 Not Found for invalid email");
