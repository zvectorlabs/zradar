...
async fn test_query_spans_with_filters() -> Result<()> {
    ...
    println!("Span query response: {}", serde_json::to_string_pretty(&body)?);
    
    // Assert we got spans back
    let data = body.get("items")
        .and_then(|d| d.as_array())
        .expect("Expected 'items' array in response");
    
    assert!(data.len() >= 3, "Expected at least 3 spans, got {}", data.len());
    ...
    for span in data {
        ...
        assert!(span.get("operation_name").is_some(), "Span should have operation_name");
        assert!(span.get("duration_ms").is_some(), "Span should have duration_ms");
    }
    ...
    let filtered_data = filtered_body.get("items")
        .and_then(|d| d.as_array())
        .expect("Expected 'items' array in filtered response");
    
    // Should only get spans with "db.query" in the name
    assert!(!filtered_data.is_empty(), "Expected at least 1 filtered span");
    for span in filtered_data {
        let name = span.get("operation_name").and_then(|n| n.as_str()).unwrap_or("");
        assert!(name.contains("db.query"), "Filtered span name should contain 'db.query', got: {}", name);
    }
...
async fn test_query_traces_with_attribute_filters() -> Result<()> {
    ...
    let body: Value = response.json().await?;
    println!("Basic query response: {}", serde_json::to_string_pretty(&body)?);
...
async fn test_error_analytics() -> Result<()> {
    ...
    if status != 200 {
        ...
        panic!("Expected 200 OK for error analytics, got {}: {}", status, error_text);
    }
    
    let body: Value = response.json().await?;
    println!("Error analytics response: {}", serde_json::to_string_pretty(&body)?);
