...
async fn test_create_score_via_rest() -> Result<()> {
    ...
    let org = client.create_organization(&fixture.org_name, &fixture.org_display_name).await?;
    let org_id = parse_uuid_from_json(&org, "id")?;
    let project = client.create_project(&org_id, &fixture.project_name, &fixture.project_display_name).await?;
...
async fn test_get_trace_scores() -> Result<()> {
    ...
    println!("🔍 DEBUG: org_id={}, project_id={}, trace_id={}", org_id, project_id, trace_id);
    
    // Create multiple scores
    for (name, value) in [("accuracy", 0.95), ("hallucination", 0.12), ("toxicity", 0.03)] {
        let response = client.post(
            &format!("/api/v1/projects/{}/scores", project_id),
            &json!({
                "trace_id": trace_id,
        ...
        if response.status() == 201 {
            ...
            println!("❌ Failed to create score {}: status={}", name, response.status());
        }
    }
    
    println!("🔍 Querying: GET /api/v1/projects/{}/traces/{}/scores", project_id, trace_id);
    
    // Query via REST API (server logs will show ClickHouse query details)
    let response = client.get(&format!("/api/v1/projects/{}/traces/{}/scores", project_id, trace_id)).await?;
    ...
    if scores.is_empty() {
        ...
        for score in &scores {
            println!("  - API Score: name={}, trace_id={}", 
...
async fn test_score_summary() -> Result<()> {
    ...
    // Get summary
    let response = client.get(&format!("/api/v1/projects/{}/traces/{}/scores/summary", project_id, trace_id)).await?;
    assert_eq!(response.status(), 200);
    ...
    assert!(avg > 0.8 && avg < 0.9, "Average should be between 0.8 and 0.9");
...
async fn test_session_scores() -> Result<()> {
    ...
    let response = client.get(&format!("/api/v1/projects/{}/sessions/{}/scores", project_id, session_id)).await?;
    assert_eq!(response.status(), 200);
...
async fn test_delete_score() -> Result<()> {
    ...
    println!("🔍 Delete URL: /api/v1/projects/{}/scores/{}", project_id, score_id);
    
    // Delete score (data should be immediately available due to insert_scores in test mode)
    let response = client.delete(&format!("/api/v1/projects/{}/scores/{}", project_id, &score_id)).await?;
    ...
    let response = client.get(&format!("/api/v1/projects/{}/scores/{}", project_id, &score_id)).await?;
...
async fn test_tenant_isolation() -> Result<()> {
    ...
    let org1 = client1.create_organization(&fixture1.org_name, &fixture1.org_display_name).await?;
    let org_id1 = parse_uuid_from_json(&org1, "id")?;
    let project1 = client1.create_project(&org_id1, &fixture1.project_name, &fixture1.project_display_name).await?;
    let project_id1 = parse_uuid_from_json(&project1, "id")?;
    
    // Setup tenant 2
    let fixture2 = TestFixture::new();
    let org2 = client2.create_organization(&fixture2.org_name, &fixture2.org_display_name).await?;
    let org_id2 = parse_uuid_from_json(&org2, "id")?;
    let project2 = client2.create_project(&org_id2, &fixture2.project_name, &fixture2.project_display_name).await?;
    let project_id2 = parse_uuid_from_json(&project2, "id")?;
    
    let trace_id = "trace_isolation_001";
    
    // Create score in tenant 1
    let response = client1.post(
    ...
    let response = client1.get(&format!("/api/v1/projects/{}/traces/{}/scores", project_id1, trace_id)).await?;
    assert_eq!(response.status(), 200);
    let tenant1_scores: Vec<serde_json::Value> = response.json().await?;
    assert_eq!(tenant1_scores.len(), 1, "Tenant 1 should see their own score");
    
    // Verify tenant 2 CANNOT see tenant 1's score (tenant isolation)
    let response = client2.get(&format!("/api/v1/projects/{}/traces/{}/scores", project_id2, trace_id)).await?;
    let scores: Vec<serde_json::Value> = response.json().await?;
...
async fn test_score_sources() -> Result<()> {
    ...
    let sources: Vec<&str> = scores.iter()
...
async fn test_bulk_score_creation() -> Result<()> {
    ...
    assert!(duration.as_secs() < 30, "Bulk creation took too long: {:?}", duration);
