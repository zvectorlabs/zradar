...
async fn test_rest_api_filter_by_span_type() -> Result<()> {
    ...
    let response = client.get(&format!("/api/v1/spans?project_id={}&span_type=GENERATION", project_id)).await?;
    
    assert_eq!(response.status(), 200, "Should be able to query with span_type filter");
    
    let data: serde_json::Value = response.json().await?;
    let spans = data["items"].as_array().expect("Should have items array");
    
    // All returned spans should be GENERATION
    assert!(!spans.is_empty(), "Should have at least one GENERATION span");
    for span in spans {
        assert_eq!(
            span["span_type"].as_str().expect("span_type should exist"),
            "GENERATION",
...
async fn test_rest_api_filter_by_multiple_span_types() -> Result<()> {
    ...
    let response = client.get(&format!("/api/v1/spans?project_id={}&span_types=GENERATION,TOOL", project_id)).await?;
    
    assert_eq!(response.status(), 200, "Should be able to query with multiple span_types");
    
    let data: serde_json::Value = response.json().await?;
    let spans = data["items"].as_array().expect("Should have items array");
    
    // All returned spans should be GENERATION or TOOL
    assert!(spans.len() >= 2, "Should have at least 2 spans (GENERATION and TOOL)");
    for span in spans {
...
fn build_trace_with_attributes(
    ...
    let span = OtlpSpan {
        ...
        attributes: attributes.into_iter().map(|(key, value)| {
            KeyValue {
                key: key.to_string(),
                value: Some(value),
            }
