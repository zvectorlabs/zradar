...
async fn test_span_storage_and_retrieval() -> Result<()> {
    ...
    let api_key = client.create_api_key(&project_id, &fixture.api_key_name, &fixture.api_key_description).await?;
    ...
    println!("📊 Retrieved trace: {}", serde_json::to_string_pretty(&trace_data)?);
    
    // Verify spans exist
    let spans = trace_data["spans"].as_array()
    ...
    // Verify span fields
    let span = &spans[0];
    assert_eq!(span["trace_id"].as_str().unwrap(), trace_id_hex, "trace_id should match");
    assert_eq!(span["service_name"].as_str().unwrap(), service_name, "service_name should match");
    assert_eq!(span["operation_name"].as_str().unwrap(), span_name, "operation_name should match");
...
async fn test_llm_span_fields_storage() -> Result<()> {
    ...
    otlp_client
        .send_test_trace("llm-service", &trace_id, &span_id, "openai.chat.completions")
        .await?;
    ...
    let trace_id_hex = hex::encode(&trace_id);
    let response = client.get(&format!("/api/v1/traces/{}?project_id={}", trace_id_hex, project_id)).await?;
...
async fn test_trace_query_with_filters() -> Result<()> {
    ...
    for service in [service1, service2] {
        let trace_id = TestDataGenerator::trace_id();
        let span_id = TestDataGenerator::span_id();
        otlp_client.send_test_trace(service, &trace_id, &span_id, "test.operation").await?;
    }
...
async fn test_telemetry_tenant_isolation() -> Result<()> {
    ...
    // Setup org1/project1
    let org1 = client.create_organization(&fixture1.org_name, &fixture1.org_display_name).await?;
    let org_id1 = parse_uuid_from_json(&org1, "id")?;
    let project1 = client.create_project(&org_id1, &fixture1.project_name, &fixture1.project_display_name).await?;
    let project_id1 = parse_uuid_from_json(&project1, "id")?;
    let api_key1 = client.create_api_key(&project_id1, &fixture1.api_key_name, &fixture1.api_key_description).await?;
    let key1 = get_string_from_json(&api_key1, "key")?;
    
    // Setup org2/project2
    let org2 = client.create_organization(&fixture2.org_name, &fixture2.org_display_name).await?;
    let org_id2 = parse_uuid_from_json(&org2, "id")?;
    let project2 = client.create_project(&org_id2, &fixture2.project_name, &fixture2.project_display_name).await?;
    ...
    otlp1.send_test_trace("isolated-service", &trace_id, &span_id, "test.isolated").await?;
    ...
    let response1 = client.get(&format!("/api/v1/traces/{}?project_id={}", trace_id_hex, project_id1)).await?;
    assert_eq!(response1.status(), 200, "Project1 should see its trace");
    
    // Query from project2 - should NOT find it
    let response2 = client.get(&format!("/api/v1/traces/{}?project_id={}", trace_id_hex, project_id2)).await?;
