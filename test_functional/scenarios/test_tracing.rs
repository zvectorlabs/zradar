...
async fn test_send_single_trace() -> Result<()> {
    ...
    assert_eq!(query_response.status(), 200, "Should be able to query stored trace");
    ...
    let spans = trace_data["spans"].as_array().expect("spans should be an array");
...
async fn test_high_volume_trace_ingestion() -> Result<()> {
    ...
    println!("✅ {} traces ingested in {:?} ({:.2} traces/sec)", 
...
async fn test_concurrent_trace_ingestion() -> Result<()> {
    ...
    for i in 0..10 {
        ...
        let handle = tokio::spawn(async move {
            ...
            otlp_client.send_test_trace(&svc, &trace_id, &span_id, &span_name).await
        });
